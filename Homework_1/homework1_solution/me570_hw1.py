...
def polygon_is_visible_test_plot(polygon, test_points):
    """
    Helper function for polygon_is_visible_test to run tests and plot the results for a single
    polygon
    """
...
def polygon_is_visible_test():
    ...
    random using np.random.rand and scaled to approximately occupy the rectangle [0,5] [-2,2]
    (i.e., the x coordinates of the points should fall between 0 and 5, while the y coordinates
    between -2 and 2).
    ...
    following:
    - Create the array  test_points_with_polygon by concatenating  test_points with the coordinates
     of the polygon (i.e., the coordinates of the polygon become also test points).
    - Plot the polygon (use Polygon.plot).
    - item:test-visibility For each vertex v in the polygon:
    - Compute the visibility of each point in  test_points_with_polygon with respect to that
    polygon (using Polygon.is_visible).
    ...
    for polygon in two_link.polygons:
...
def polygon_is_collision_test():
    ...
    the following:
    - Compute whether each point in  test_points_with_polygon is in collision with the polygon or
    not using Polygon.is_collision.
    - Plot each point in  test_points_with_polygon in green if it is not in collision, and red
    otherwise.  Moreover, increase the number of test points from 5 to 100 (i.e.,  testPoints
    should have dimension [2 x 100]).
    """
...
def priority_test():
    ...
    print(
        f"priority_test: Oranges is in queue? --> {my_queue.is_member('Oranges')}"
    )
